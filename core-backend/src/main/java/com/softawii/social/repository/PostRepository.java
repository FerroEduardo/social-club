...
public interface PostRepository extends JpaRepository<Post, Long> {
    @Query(value = """
            ...
            WHERE (:post_id IS NULL OR :post_id = p.id) AND (:user_id IS NULL OR :user_id = u.id)
            """,
           countQuery = "SELECT COUNT(p) FROM social.post p INNER JOIN social.user u ON u.id = p.author_id WHERE (:post_id IS NULL OR :post_id = p.id) AND (:image_url = :image_url) AND (:user_id IS NULL OR :user_id = u.id)",
           nativeQuery = true)
